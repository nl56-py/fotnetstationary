...
function luzuk_mts_load_widget() {
...
class Social_Profile_Icons_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			...
			'description' => __( 'Show profile icons.', 'Luzuk Premium' ),
		);
		...
		parent::__construct( 'social-profile-icons', sprintf( __( '%s Social Profile Icons', 'Luzuk Premium' ), 'MTS' ), $widget_ops, $control_ops );
	...
	function form( $instance ) {
		...
		<p><label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php _e( 'Title:', 'Luzuk Premium' ); ?></label> <input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $instance['title'] ); ?>" /></p>

		<p><label><input id="<?php echo esc_attr($this->get_field_id( 'new_window' )); ?>" type="checkbox" name="<?php echo $this->get_field_name( 'new_window' ); ?>" value="1" <?php checked( 1, $instance['new_window'] ); ?>/> <?php esc_html_e( 'Open links in new window?', 'Luzuk Premium' ); ?></label></p>
...
add_action( 'widgets_init', 'luzuk_mts_load_widget' );